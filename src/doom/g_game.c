...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic)
{
    ...
    speed = key_speed >= NUMKEYS || joybspeed >= MAX_JOY_BUTTONS || gamekeydown[key_speed] ||
            gamekeydown[key_speed_alt] || joybuttons[joybspeed];

    forward = side = 0;

    // use two stage accelerative turning
    // on the keyboard and joystick
    if (joyxmove < 0 || joyxmove > 0 || gamekeydown[key_right] || gamekeydown[KEY_RIGHTARROW] ||
        gamekeydown[key_left] || gamekeydown[KEY_LEFTARROW] || mousebuttons[mousebturnright] ||
    ...
    if (strafe) {
        if (gamekeydown[key_right] || gamekeydown[KEY_RIGHTARROW] || mousebuttons[mousebturnright]) {
            // fprintf(stderr, "strafe right\n");
            side += sidemove[speed];
        }
        if (gamekeydown[key_left] || gamekeydown[KEY_LEFTARROW] || mousebuttons[mousebturnleft]) {
