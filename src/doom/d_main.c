...
boolean D_Display(void)
{
    ...
    case GS_LEVEL:
        ...
        if (wipe || (viewheight != SCREENHEIGHT && fullscreen)) redrawsbar = true;
    ...
    if (gamestate == GS_LEVEL && !automapactive && scaledviewwidth != SCREENWIDTH) {
        if (menuactive || menuactivestate || !viewactivestate) borderdrawcount = 3;
        if (borderdrawcount) {
...
void D_DoomMain(void)
{
    ...
    if (modifiedgame && (gamevariant != freedoom)) {
        ...
        if (gamemode == registered)
            for (i = 0; i < 23; i++)
                if (W_CheckNumForName(name[i]) < 0) I_Error(DEH_String("\nThis is not the registered version."));
    }
